...
export function getCookie(name: string): string | undefined {
  ...
  // ex) document.cookie = "user%20name=John; theme=light" -> ["user%20name=John", "theme=light"]
  const cookies = document.cookie.split("; ");
  // 3. 배열에서 존재하는지 찾기 (encodedName=으로 시작하는지 확인)
  // ex) "user name"라는 키를 가진 쿠키를 찾는다. ["user%20name=John", "theme=light"] -> "user%20name=John"
  const cookie = cookies.find((cookie) => cookie.startsWith(encodedName + "="));
  // 4. 있으면 디코딩해서 값 변환
  if (cookie) {
    // ex) "user%20name=John" -> "John"
